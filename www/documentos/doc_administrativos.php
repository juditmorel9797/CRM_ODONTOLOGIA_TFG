...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['subir_documento'])) {
    ...
    if (isset($_FILES['archivo']) && $_FILES['archivo']['error'] == 0) {
        ...
        $sql = "INSERT INTO documento_administrativo (uuid, id_paciente, tipo, archivo_nombre, archivo_tipo, archivo, subido_por)
                VALUES ('$uuid', '$id_paciente', '$tipo', '$nombre', '$tipo_mime', '$contenido', '$id_usuario')";
        if ($conn->query($sql) === TRUE) {
            header("Location: doc_administrativos.php?id_paciente=$id_paciente&ok=1");
            exit;
        } else {
...
<main class="container">
    ...
    <form action="doc_administrativos.php?id_paciente=<?= $id_paciente ?>" method="POST" enctype="multipart/form-data" style="display:flex;gap:12px;align-items:end;margin-bottom:20px;">
        ...
        <label>
            ...
            <select name="tipo" required>
                ...
                <option value="Presupuesto firmado">Presupuesto firmado</option>
                <option value="Justificante de pago">Justificante de pago</option>
    ...
    <table class="historial-table">
        ...
        <tbody>
            <?php
            $res = $conn->query("SELECT * FROM documento_administrativo WHERE id_paciente='$id_paciente' ORDER BY fecha_subida DESC");
            ...
            <tr>
                ...
                <td>
                    <a href="descargar_archivo.php?tabla=documento&id=<?= $row['id'] ?>" class="chat-add-btn" style="background:#5CA9E9;padding:4px 12px;font-size:14px;">Visualizar</a>
                </td>
            </tr>
            <?php endwhile; ?>
        </tbody>
